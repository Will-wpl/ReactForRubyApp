                ...
                if (data && data.action && data.action === ACTION_COMMANDS.SET_BID) {
                    ...
                    if (last) {
                        ...
                        // if (last.is_bidder) {
                            ...
                            last.template_ranking = `Ranking: ${Number(last.ranking) <= 2 ? 'TOP 2' : getNumBref(last.ranking)} ${last.is_bidder && last.flag !== null ? '(Bit Submitter)' : ''}`;
    ...
    makeup(res) {
        if (res.length > 0) {
            ...
            copy.forEach(history => {
                ...
                history.template_ranking = `Ranking: ${Number(history.ranking) <= 2 ? 'TOP 2' : getNumBref(history.ranking)} ${history.is_bidder && history.flag !== null  ? '(Bit Submitter)' : ''}`;
