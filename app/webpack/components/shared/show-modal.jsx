            ...
            if(this.props.listdetailtype === "Select Retailers"){
                ...
                showDetail = <ul className="showdetail history_files">
                                {this.props.listdetail.map((item,index)=>{
                                  return <li key={index}><a target="_blank" download={item.file_name} href={item.file_path}>{item.file_name}</a><abbr> | {item.file_time}</abbr><span className="remove_file" onClick={this.removefile.bind(this,this.state.strtype,index,item.fileid)}></span></li>
                                })}
                            </ul>
