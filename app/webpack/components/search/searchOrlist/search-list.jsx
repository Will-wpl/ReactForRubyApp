                                        ...
                                        return <tr key={index}>
                                            {
                                                this.props.table_data.headers.map((it, i) => {
                                                    ...
                                                    else if (it.field_name === 'accept_status' && it.table_name === 'request_auctions') {
                                                        return <td key={i}>
                                                            {item[`${it.field_name}`] === null ? 'Pending' : (item[`${it.field_name}`] === '0' ? 'Rejected' : (item[`${it.field_name}`] === '1' ? 'Accepted' : "Pending"))}
                                                        </td>
                                                        ...
                                                        return <td key={i}>
                                                            {
                                                                item.contracts ? item.contracts.map((k, s) => {
                                                                    ...
                                                                    : <abbr>
                                                                        {(it.field_name === "actual_begin_time" || it.field_name === "start_datetime" || it.field_name === "logged_in_last_time" || it.field_name === "ws_connected_last_time" || it.field_name === "ws_send_message_last_time" || it.field_name === "auction_when" || it.field_name === "updated_at")

                                                                            ? moment(item[`${it.field_name}`]).format('D MMM YYYY hh:mm A')
                                                                            : ((it.field_name === "contract_period_start_date") ? moment(item[`${it.field_name}`]).format('D MMM YYYY ') : (item[`${it.field_name}`] ? (item[`${it.field_name}`].indexOf("$e_x@c%l^m&a*r!k$") > 0 ? <bdo>{item[`${it.field_name}`].split("$e_x@c%l^m&a*r!k$")[0]} <b title="Purchasing Entity Pending" style={{ "color": "red" }}>!</b></bdo> : item[`${it.field_name}`]) : null))}</abbr>
                                                            }
                                                        </td>
                                                    }
                                                            ...
                                                            if (ik.check === "docheck") {
                                                                ...
                                                                if (item['auction_status'] === 'Upcoming' && ik.name === 'Manage') {
                                                                    ...
                                                                    if (item['status'] === 'In Progress' && ik.name === 'Manage') {
                                                                        ...
                                                                        if(item.show_dashboard==true && item['publish_status'] === '1'){
