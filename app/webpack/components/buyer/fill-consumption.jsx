                    ...
                    ? this.props.site_list.map((item, index) => <div key={item.cid}>
                             ...
                             <table className="retailer_fill u-mb3" cellPadding="0" cellSpacing="0">
                                 <tbody>
                                     ...
                                     <tr><td>Contracted Capacity</td><td><input type="text" disabled={this.props.checked} required aria-required="true" name={"capacity"+(index+1)} id={"capacity"+(index+1)} pattern="^(0|[1-9][0-9]*)$" defaultValue={item.contracted_capacity ? formatPower(parseInt(Number(item.contracted_capacity)), 0, '') : ''}/></td></tr>}
                                     <tr><td>Peak Volume (kWh/month)</td><td><input type="text" disabled={this.props.checked} required aria-required="true" name={"peak"+(index+1)} id={"peak"+(index+1)} defaultValue={item.peak ? (this.props.checked?formatPower(parseInt(Number(item.peak)), 0, ''):item.peak) : ''} pattern="^(0|[1-9][0-9]*)$" title="Must be positive integers, and the first cannot be 0."/></td></tr>
                                     <tr><td>Off-Peak Volume (kWh/month)</td><td><input type="text" disabled={this.props.checked} required aria-required="true" name={"off_peak"+(index+1)} defaultValue={item.off_peak ? (this.props.checked?formatPower(parseInt(Number(item.off_peak)), 0, ''):item.off_peak) : ''} id={"off_peak"+(index+1)} pattern="^(0|[1-9][0-9]*)$" title="Must be positive integers, and the first cannot be 0."/></td></tr>
                                 </tbody>
                             </table>
                          </div>)
                    : ''
