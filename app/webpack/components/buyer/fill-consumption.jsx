            ...
            <div>
                {
                    ...
                    return <div key={index}>
                             ...
                             <table className="retailer_fill u-mb3" cellPadding="0" cellSpacing="0">
                                 <tbody>
                                     <tr><td>Accound Number</td><td><input type="text" defaultValue={`${item.number}${index+1}`} id={"number"+(index+1)} name={"number"+(index+1)}/></td></tr>
                                     <tr><td>Intake Level</td>
                                        <td>
                                            <select id={"level"+(index+1)} name={"level"+(index+1)}>
                                            {
                                                this.props.site_list[index].level.map((it,i)=>{
                                                    return <option key={i} defaultValue={i+1}>{it}</option>
                                                })
                                            }
                                            </select>
                                        </td></tr>
                                     <tr><td>Peak Volume (kWh/mouth)</td><td><input type="text" name={"peak"+(index+1)} id={"peak"+(index+1)} defaultValue={item.peak}/></td></tr>
                                     <tr><td>Off-peak Volume (kWh/mouth)</td><td><input type="text" name={"off_peak"+(index+1)} defaultValue={item.off_peak} id={"off_peak"+(index+1)}/></td></tr>
                                 </tbody>
                             </table>
                          </div>
                    }) 
