                    ...
                    ? this.props.site_list.map((item, index) => <div key={item.cid}>
                             ...
                             <table className="retailer_fill u-mb3" cellPadding="0" cellSpacing="0">
                                 <tbody>
                                     ...
                                     <tr><td>Contracted Capacity (kW)</td><td><input type="text" disabled={this.props.checked} required aria-required="true" name={"capacity"+(index+1)} id={"capacity"+(index+1)} pattern="^(0|[1-9][0-9]*)$" defaultValue={item.contracted_capacity ? (this.props.checked?formatPower(parseInt(Number(item.contracted_capacity)), 0, ''):parseInt(Number(item.contracted_capacity))) : ''}/></td></tr>}
