...
import {buyerSendResponse,getbuyerDeviations,removebuyerFile} from '../../../javascripts/componentService/common/service';
import {getTenderhistory} from '../../../javascripts/componentService/common/service';
export class BuyerKeppelproposedeviations extends Component {
    ...
    showhistory(id){
        ...
        getTenderhistory('buyer',id).then(res=>{
            //console.log(res);
    ...
    send_response(){
        buyerSendResponse(this.props.current.current.arrangement_id,this.editData()).then(res=>{
            this.refs.Modal.showModal();
            this.setState({
                text:"Response successfully sent."
            });
            setTimeout(()=>{
                window.location.href="/buyer/auctions/"+sessionStorage.auction_id+"/retailer_dashboard";
            },3000)
    ...
    upload(type, index){
        ...
        $.ajax({
            url: '/api/buyer/auction_attachments?auction_id='+sessionStorage.auction_id+'&file_type='+type+'&user_id='+this.state.user_id,
            type: 'POST',
            cache: false,
            ...
            do_remove(callbackObj){
                ...
                let fileObj;
                removebuyerFile(callbackObj.fileid).then(res=>{
                    fileObj = this.state.fileData;
