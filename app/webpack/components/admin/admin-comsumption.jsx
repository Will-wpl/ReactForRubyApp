    ...
    componentDidMount() {
        ...
        getBuyerDetails({ id: sessionStorage.auction_id, type: this.datail_type, role: role, contract_duration: window.location.href.split('duration=')[1] }).then(res => {
            this.setState({
                ...
                dataVersion: window.location.href.split('duration=')[1] ? "1" : "",
            })
        }, error => {
        ...
        return (
            <div className="u-grid mg0">
                <div className="col-sm-12 u-mb3">
                    <AdminComsumptionList dataVersion={this.state.dataVersion} visible={true} table={this.state.detail} detail_index={this.state.detail_index} type={this.pageType} comsumption_list={this.state.comsumption_list} detail={this.show_detail.bind(this)} />
                </div>
