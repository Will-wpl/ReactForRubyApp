class BuyerEntityReport < BuyerReport
  ...
  def pdf_draw_info(pdf, current_user, entities)
    ...
    info_data.push(["<b>Contract Period: </b>", "#{contract_period_start_date} to #{contract_period_end_date} (#{auction_result.contract_duration} months)"])
    entity_list_table = pdf.make_table(get_entities_list(entities), :cell_style => { :size => 14, :align => :left, :valign => :top, :padding => [0, 2, 0], :inline_format => true, :border_width => 0.0})
