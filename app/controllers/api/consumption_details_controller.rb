class Api::ConsumptionDetailsController < Api::BaseController
    ...
    unless params[:consumption_id].nil?
      ...
      if auction.auction_contracts.blank?
        ...
        if auction.tc_attach_info.blank?
          seller_buyer_tc_attachment = UserAttachment.find_last_by_type(UserAttachment::FileType_Seller_Buyer_TC) if auction.request_auction_id.blank?
          buyer_revv_tc_attachment = UserAttachment.find_last_by_type(UserAttachment::FileType_Buyer_REVV_TC)
        else
          if auction.request_auction_id.blank?
  ...
  def validate_duplicated_consumption_detail(detail)
    ...
    if detail_id.nil?
      ...
      duplicated_account_details = ConsumptionDetail.find_duplicated_account_number_except_id(validated_detail['account_number'], detail_id)
    end
    ...
    if detail_id.nil?
      duplicated_address_details = ConsumptionDetail.find_duplicated_address(validated_detail['unit_number'], validated_detail['postal_code'])
    else
      duplicated_address_details = ConsumptionDetail.find_duplicated_address_except_id(validated_detail['unit_number'],
