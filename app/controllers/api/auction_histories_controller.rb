class Api::AuctionHistoriesController < Api::BaseController
  ...
  def show
    @histories = AuctionHistory.select('users.name, users.company_name, auction_histories.*').joins(:user).where('auction_id = ? and user_id = ?', params[:auction_id], current_user.id).order(actual_bid_time: :asc)
    render json: @histories, status: 200
  end

  # get all users history list by auction_id
  def list
    @histories = AuctionHistory.select('users.name, users.company_name, auction_histories.*').joins(:user).where('auction_id = ?', params[:auction_id]).order(actual_bid_time: :asc)
